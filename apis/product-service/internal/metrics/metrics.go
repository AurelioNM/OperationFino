...
func NewProductMetrics(l slog.Logger, reg prometheus.Registerer) *ProductMetrics {
...
func (m *ProductMetrics) IncReqByStatusCode(status string) {
	m.ReqByStatusCode.With(prometheus.Labels{"service": m.service, "status": status}).Inc()
}

func (m *ProductMetrics) MeasureDuration(start time.Time, method string, uri string, statusCode string) {
...
func (m *ProductMetrics) MeasureExternalDuration(start time.Time, resource string, method string, uri string, statusCode string) {
